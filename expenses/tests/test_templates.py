...
class UpdateBudgetTest(TestCase):
  ...
  def setUp(self):
      ...
      self.response = self.client.get(reverse('expenses:update_budget'), kwargs={'pk': test_budget.pk})
...
class Test404Page(TestCase):
  ...
  def test_404page_contains_correct_html(self):
      self.assertContains(self.response, 'The item you requested is not available. (404)')
